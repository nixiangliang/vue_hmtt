<template>
  <div>
    ...
    <div class="search-header">
      ...
      <!-- 搜索组件 -->
      <van-search v-model.trim="kw" v-fofo placeholder="请输入搜索关键词" background="#007BFF" shape="round" @input="inputFn" @search="searchFn"/>
    </div>
  ...
  methods: {
    ...
    inputFn () {
      ...
      if (this.kw.length === 0) {
        ...
        this.timer = setTimeout(async () => {
          if (this.kw.length === 0) return // 输入框为空，不进行打印
    ...
    lightFn (originStr, target) {
      ...
      if (originStr !== null) {
        ...
        // 例如：输入框里java，匹配回来的联想菜单是Java,JAVA,jAVA,都被你replace换成输入框target值java
    ...
    moveFn (theKw) {
      // 路由跳转传参
      // 方式1：路径/值(前提：路由规则:变量名)，-> 接受：$route.params
