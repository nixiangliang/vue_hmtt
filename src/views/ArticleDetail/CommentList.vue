<template>
  <div>
    <!-- 评论列表 -->
    <!-- 1. van-list最好在这个div内，让底部文字/加载中，作为此div内容，容器padding-bottom底部文字也挤上去了-看得见 -->
    <div class="cmt-list" :class="{ 'art-cmt-container-1': isShowCmtBox, 'art-cmt-container-2': !isShowCmtBox }">
      <!-- list列表 加载更多 -->
      <!-- 2. van-list包裹列表，组件内源码，监测网页滚动事件，判断滚动位置是否快到达内容高度(触底)
...
export default {
  ...
  async created () {
    ...
    console.log(res)
    this.commentList = res.data.data.results // 前10条评论列表
    this.totalCount = res.data.data.total_count // 总评论数量
  ...
  methods: {
    ...
    // 底部加载更多
    async onLoad () {
      // 判断和immediate-check可选任意一个(所以这里不写)
      ...
      // 请求下一页数据
        ...
        this.commentList = [...this.commentList, ...res.data.data.results] // 前10条评论列表
        this.totalCount = res.data.data.total_count // 总评论数量
        this.lastId = res.data.data.last_id // 分页
    ...
    // 收藏按钮-点击事件
    // async collectClickFn (bool, obj) {
    //   this.isShowColIcon = !this.isShowColIcon
    //   if (this.isShowColIcon === true) {
    //     // 用户点在了 '取消收藏' 图标上，页面显示 '收藏' 图标
    //     // 业务接口 -> 收藏文章
    ...
    //     console.log(res)
    //   } else {
    //     // 用户点在了 '收藏' 图标上，页面显示 '取消收藏' 图标
    //     // 业务接口 -> 取消收藏文章
    //     const res = await unCollectArticleAPI({
    //       artId: this.$route.query.art_id
    //     })
    //     console.log(res)
    //   }
...
// 分享到微博 -> 微博开发平台 -> 注册账号，调用人家平台封装额js代码，调用他们的接口来实现分享的功能
</script>
