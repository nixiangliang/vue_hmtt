      ...
      <template #title>
        ...
        <div class="thumb-box" v-if="artObj.cover.type === 3">
          <img
            ...
            alt=""
          />
          <!-- <van-image v-for="(imgUrl, index) in artObj.cover.images" :key="index" class="thumb" v-lazy="imgUrl">
            <template v-slot:error>图片走丢了</template>
          </van-image> -->
        </div>
      </template>
...
export default {
  props: {
    ...
    isShow: {
      type: Boolean,
      default: true // 例如首页的地方我不想再去动了，首页肯定没传isShow值进来，所以首页需要显示
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    onSelect(action, item) {
      ...
      if (action.name === '反馈垃圾内容') {
        ...
        this.show = false // 反馈-下面cancel会帮你把反馈选项切回来
      }
    },
    // 反馈面板-底部按钮-点击事件
    cancelFn() {
